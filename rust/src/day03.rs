...
pub fn part1() -> Result<u32, Box<dyn std::error::Error>> {
    ...
    let common_item_in_various_rucksack_compartment = lines.into_iter().map(|line| {
        let midpoint = line.len().midpoint(0);
        let (first_compartment, second_compartment) = line.split_at(midpoint);
    ...
    let priority_sum = items_to_priority(common_item_in_various_rucksack_compartment);
...
{
    let item_priority = common_item_in_various_rucksack_compartment.map(|item| {
...
pub fn part2() -> u32 {
    ...
    let group_badges = lines.array_chunks::<3>().map(|[first, second, thrid]| {
        let mut common_badge_item = first.chars().filter(|&items| second.contains(items));
        let next_common_badge_item = second
            .chars()
            .filter(|&items| thrid.contains(items))
            .collect::<Vec<char>>();

        let badge = common_badge_item.find(|badge| next_common_badge_item.contains(badge));
        badge.unwrap()
    });
